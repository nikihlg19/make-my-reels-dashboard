...
const ProjectCardInner: React.FC<ProjectCardProps> = ({
  ...
  const handleSendToClient = (e: React.MouseEvent, client: Client) => {
    e.stopPropagation();
    setShowConfirmPopover(false);
    ...
    const amountLine = (project.invoice_amount && project.invoice_amount > 0)
      ? `₹${project.invoice_amount.toLocaleString()}`
      : 'TBD';

    const text = [
      `Hi ${client.name || client.company || 'there'}! 👋`,
      '',
      `🎬 *Booking Confirmation: ${project.title}*`,
      '',
      `📅 Date: ${formatDisplayDate(project.eventDate)}${project.eventTime ? ` @ ${project.eventTime}` : ''}`,
      `📍 Location: ${parsedLoc.address}`,
      `🚗 Map: ${mapsLink}`,
      `🎯 Services: ${project.description || 'Photography & Reels'}`,
      `👥 Team: ${teamLine}`,
      `💰 Amount: ${amountLine}`,
      `💳 UPI: ${upiId}`,
    ].join('\n');
    ...
    >
      ...
      <div className="flex justify-between items-start mb-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* WhatsApp button: tap to chat, long-press for send details popup */}
            ...
            <div className="relative" ref={popoverRef}>
              <button
                ...
                onClick={(e) => {
                  ...
                  const phone = projectClients[0].phone.replace(/[^0-9]/g, '');
                  window.open(`https://wa.me/${phone}`, '_blank');
                }}
                className="text-emerald-400 hover:text-emerald-500 transition-colors mr-1"
                title={`WhatsApp ${projectClients[0].name || projectClients[0].company}`}
              >
              ...
              {/* Send details popover (shown on long press) */}
              {showConfirmPopover && (
                <div
