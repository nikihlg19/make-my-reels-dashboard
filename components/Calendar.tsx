  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-white overflow-hidden pb-16 md:pb-0">
      ...
      <div className="flex-1 overflow-hidden min-h-0 flex flex-col">
        <div className="grid grid-cols-7 border-b border-slate-200 shrink-0 bg-slate-50/30 border-l border-slate-200">
          {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map((day, i) => (
            <div key={day} className="py-1.5 md:py-2 text-center text-[10px] font-black text-slate-400 md:tracking-[0.2em] border-r border-slate-200">
        ...
        <div className="flex-1 grid grid-cols-7 border-l border-slate-200" style={{ gridTemplateRows: `repeat(${Math.ceil(calendarDays.slice(0, 42).length / 7)}, 1fr)` }}>
          ...
          return (
            <div
              ...
              className={`p-0.5 md:p-1.5 border-r border-b border-slate-200 hover:bg-indigo-50/20 transition-colors cursor-pointer flex flex-col items-center md:items-start gap-0.5 md:gap-1 overflow-hidden ${!isCurrentMonth ? 'bg-slate-50/40 grayscale opacity-60' : isTodayDate ? 'bg-indigo-50/60 border-indigo-100/40' : 'bg-white'}`}
            >
              <div className="flex flex-col md:flex-row items-center justify-between w-full">
                <span className={`w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-full text-[11px] md:text-xs font-black transition-all ${isTodayDate ? 'bg-[#4F46E5] text-white shadow-lg scale-105' : hasHoliday ? 'text-emerald-600' : isCurrentMonth ? 'text-slate-800' : 'text-slate-400'}`}>
                  {format(day, 'd')}
                </span>
                {dayEvents.length > 0 && (
                  <span className="hidden md:flex text-[7px] font-black text-sky-600 bg-sky-50 px-1 py-0.5 rounded-md min-w-[14px] items-center justify-center border border-sky-100/50 shadow-sm">
                    {dayEvents.length}
                  </span>
                )}
              </div>
              ...
              <div className="hidden md:flex flex-1 overflow-hidden flex-col gap-0.5 w-full min-w-0">
                {dayEvents.slice(0, 2).map((event: any, i) => (
                  <div key={i} className={`text-[7px] px-1 py-0.5 rounded-md truncate font-black uppercase tracking-tighter border shadow-sm flex items-center gap-0.5 ${event.type === 'shoot' ? 'bg-rose-500 text-white border-rose-600' : event.type === 'project' ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
                    {event.type === 'shoot' && <Video size={8} className="shrink-0" />}
                    <span className="truncate">{event.title}</span>
                  </div>
                ))}
                {dayEvents.length > 2 && (
                  <div className="text-[6px] text-slate-400 font-bold pl-1">+{dayEvents.length - 2} more</div>
                )}
              </div>
    ...
    {/* Legend */}
    <div className="shrink-0 flex flex-wrap items-center justify-center gap-4 md:gap-6 px-4 py-2 bg-white border-t border-slate-200">
